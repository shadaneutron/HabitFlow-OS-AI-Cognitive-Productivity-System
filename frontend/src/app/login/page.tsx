...
import { Sparkles, LogIn, Lock, Mail, Loader2, ArrowRight } from "lucide-react";
...
export default function LoginPage() {
  ...
  const { setToken, setUser, loadSyncedState, setGuestMode, habits, matrixTasks, pomodoroSessions, inboxItems } = useAppStore();
    ...
    try {
      ...
      try {
        ...
        if (syncResponse.ok) {
          ...
          const hasCloudData = 
            syncData.habits.length > 0 || 
            syncData.matrixTasks.length > 0 || 
            syncData.pomodoroSessions.length > 0 || 
            syncData.inboxItems.length > 0;

          if (hasCloudData) {
            // Restore from cloud
            loadSyncedState({
              habits: syncData.habits,
              matrixTasks: syncData.matrixTasks,
              pomodoroSessions: syncData.pomodoroSessions,
              inboxItems: syncData.inboxItems,
              userMemory: syncData.userMemory
            });
          } else {
            // Push current guest data to cloud
        ...
        console.error("Auto-sync error during login:", syncErr);
      }
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-tr from-slate-50 via-indigo-50/30 to-slate-100 dark:from-[#0F0C20] dark:via-[#151030] dark:to-[#0A0718]">
      <div className="w-full max-w-md p-8 rounded-3xl bg-white/70 dark:bg-white/[0.02] border border-white/40 dark:border-white/[0.08] backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.04)] dark:shadow-[0_32px_64px_rgba(0,0,0,0.8)] relative overflow-hidden">
        ...
        <div className="flex flex-col items-center mb-8 relative z-10">
          ...
          <p className="text-xs text-slate-500 dark:text-[#8E8EA8] mt-1.5">Reconnect to your HabitFlow workspace</p>
        </div>
        ...
        <div className="mt-8 pt-6 border-t border-slate-200 dark:border-white/[0.06] text-center relative z-10">
          <p className="text-xs text-slate-500 dark:text-[#8E8EA8]">
            ...
            <Link href="/register" className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 font-bold transition-colors inline-flex items-center gap-0.5">
